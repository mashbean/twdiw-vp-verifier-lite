  ...
  <main>
    ...
    <section id="try" class="try-section" aria-labelledby="try-title">
      ...
      <div class="builder" aria-labelledby="builder-title">
        ...
        <div class="wallet-picker">
          ...
          <details id="bonds-picker" class="bonds-picker">
            <summary>有備而來相容測試</summary>
            <p>有備而來尚未公開上架。展開後可測試收到的政府卡片，以及新版自發證件。</p>
            <button id="wallet-bonds" class="wallet-choice" type="button" aria-pressed="false"><strong>改用有備而來</strong><small>開發中 App · 需要既有測試安裝</small></button>
          </details>
        </div>
    ...
    <section id="questions" class="faq" aria-labelledby="faq-title">
      ...
      <div class="faq-list">
        ...
        <details>
          <summary>部署完成就成為官方註冊驗證者嗎？</summary>
          <p>不會。一鍵部署只建立一套獨立的技術查驗服務。需要官方驗證者身分或正式介接時，仍須另走數位憑證皮夾的<a href="https://www.wallet.gov.tw/apply/applyIssuerVerifier.html">發行者／驗證者申請流程</a>；本專案不代辦、不代表申請通過，也與官方註冊無關。</p>
        </details>
      </div>
    </section>
...
.hero .lede{color:var(--ink);font-size:clamp(1.35rem,2.8vw,2rem);font-weight:760;line-height:1.4}.hero-supporting{max-width:830px;margin:9px 0 0;color:var(--muted);font-size:1.05rem}.wallet-picker{display:grid;grid-template-columns:1fr 1fr;gap:12px}.wallet-choice{appearance:none;width:100%;border:1px solid var(--line);border-radius:16px;background:white;padding:18px;text-align:left;cursor:pointer}.wallet-choice strong,.wallet-choice small{display:block}.wallet-choice small{margin-top:5px;color:var(--muted)}.wallet-choice[aria-pressed=true]{border-color:var(--green);box-shadow:inset 0 0 0 1px var(--green);background:#fbfffd}.bonds-picker{border:1px solid var(--line);border-radius:16px;padding:0 18px;background:#f6f8f6}.bonds-picker summary{padding:18px 0;cursor:pointer;font-weight:780}.bonds-picker p{margin:0 0 14px;color:var(--muted);font-size:.9rem}.bonds-picker .wallet-choice{margin:0 0 18px}.profile-heading{margin-top:42px}.result.warning .result-icon{background:#fff0d6;color:#8b5700}.result.warning .policy{border-left-color:var(--amber)}
...
@media(prefers-color-scheme:dark){:root{color-scheme:dark;--ink:#edf6f1;--ink-soft:#d6e8df;--muted:#adbbb4;--paper:#0d1511;--card:#142019;--line:#304239;--green:#75ddb2;--green-2:#183c2d;--shadow:none}.brand-mark,.primary,.primary-link{background:#edf6f1;color:#132019}.profile,.source,.secondary,.wallet-choice{background:#142019}.profile[aria-pressed=true],.source[aria-pressed=true],.wallet-choice[aria-pressed=true]{background:#192c23}.bonds-picker{background:#101a15}.disclosure,.policy{background:#1b2b23}.claim{background:#25372e}.qr-shell{background:white}.business-card,.implementation{background:#08110d}.citizen-card,.resource-links a,.trust-strip div{background:#142019}.developer-card{background:#14271e}.developer-card.featured{background:#edf6f1;color:#132019}.secondary-link{border-color:#edf6f1}.prompt-block{background:#0a1711}.prompt-block pre{background:#050c09}}
`;

export const FRONTEND_JS = /* js */ `
const state={profiles:[],profile:null,source:null,wallet:'twdiw',socket:null,resultReceived:false};
...
function visibleProfiles(){return state.profiles.filter((profile)=>variantsFor(profile).length)}

function chooseWallet(wallet){
  state.wallet=wallet;
  const profiles=visibleProfiles();
  if(!profiles.includes(state.profile))state.profile=profiles.find((item)=>item.id==='identity-name')||profiles[0];
...
function renderWallets(){
  ...
  if(state.wallet==='bonds')$('bonds-picker').open=true;
}
...
function renderProfiles(){
  ...
  visibleProfiles().forEach((profile)=>{
    ...
    button.onclick=()=>{state.profile=profile;state.source=variantsFor(profile)[0].source;syncURL();renderAll()};host.append(button);
  });
}
...
function renderAll(){renderWallets();renderProfiles();renderSources();renderDisclosure()}
...
async function load(){
  ...
  const available=visibleProfiles();
...
async function createPresentation(){
  if(state.socket)state.socket.close();state.socket=null;state.resultReceived=false;$('create').disabled=true;$('create-error').textContent='';
  try{
