...
零持久化不代表沒有蒐集個人資料。頁面已在建立 QR 前加入隨 profile 更新的個人資料蒐集、處理及利用告知，列出蒐集者、目的、類別、期間、地區、對象、方式、權利及拒絕影響；未確認前不能建立 QR。

示範站不保存勾選紀錄或 consent receipt，因此不能提供事後交易證明。正式、高風險或需要稽核的服務須另行確認個資法第 19 條合法事由，並設計能證明告知版本而不擴張 credential／claims 留存的機制。自行部署者也必須更換 `src/privacy-notice.ts` 的營運者與利用範圍；模板本身不是法律意見或合規認證。
